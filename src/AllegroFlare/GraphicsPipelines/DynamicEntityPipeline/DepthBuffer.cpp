...
void DepthBuffer::set_entity_pool(AllegroFlare::GraphicsPipelines::DynamicEntityPipeline::EntityPool* entity_pool)
{
   this->entity_pool = entity_pool;
}


void DepthBuffer::set_casting_light(AllegroFlare::Camera3D casting_light)
{
   this->casting_light = casting_light;
}


AllegroFlare::GraphicsPipelines::DynamicEntityPipeline::EntityPool* DepthBuffer::get_entity_pool() const
{
   return entity_pool;
}
...
AllegroFlare::Camera3D &DepthBuffer::get_casting_light_ref()
{
   return casting_light;
}
...
void DepthBuffer::setup_result_surface_bitmap(int width, int height)
{
...
void DepthBuffer::init_shader()
{
   if (!((!depth_map_shader)))
   {
      ...
      error_message << "[DepthBuffer::init_shader]: error: guard \"(!depth_map_shader)\" not met.";
...
void DepthBuffer::init_camera_defaults()
{
...
void DepthBuffer::setup_projection_on_render_surface()
{
   //casting_light.setup_projection_on(render_surface.obtain_surface());
   //return;
...
AllegroFlare::MultitextureModel3D* DepthBuffer::get_multitexture_model_3d(AllegroFlare::GraphicsPipelines::DynamicEntityPipeline::Entities::Base* entity)
{
   if (!(entity))
   {
      std::stringstream error_message;
      error_message << "[DepthBuffer::get_multitexture_model_3d]: error: guard \"entity\" not met.";
      std::cerr << "\033[1;31m" << error_message.str() << " An exception will be thrown to halt the program.\033[0m" << std::endl;
      throw std::runtime_error("DepthBuffer::get_multitexture_model_3d: error: guard \"entity\" not met");
   }
...
void DepthBuffer::render_multitexture_model_3d(AllegroFlare::MultitextureModel3D* multitexture_model_3d)
{
   // Render our subject
